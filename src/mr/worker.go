...
func DoMapTask(task *Task, mapf func(string, string) []KeyValue) {
	...
	for i := 0; i < NReduce; i++ {
		file, err = os.CreateTemp("tmp", "mr-tmp-")

		//file, err := os.Create(filename)
		if err != nil {
			log.Fatalf("[Worker] Failed to create map tmp file.", err)
		}
		enc := json.NewEncoder(file)
		for _, kv := range hash[i] {
			err := enc.Encode(kv)
			if err != nil {
				log.Fatalf("[Worker] cannot encode %v", kv)
			}
		}

		filename = fmt.Sprintf("tmp/mr-%d-%d", task.ID, i)
...
func DoReduceTask(task *Task, reducef func(string, []string) string) {
	...
	for _, filename := range task.InputFile {
		...
		if err != io.EOF {
			log.Fatalf("[Worker] cannot decode %v", kv)
		}
		file.Close()
	}
	...
	ofile, err := os.CreateTemp("tmp", "mr-out-")
	if err != nil {
		log.Fatalf("[Worker] Failed to create map tmp file.", err)
	}
